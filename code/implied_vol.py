...
Also implements the gradient-squash transformation (Section 4.3).
"""
...
def newton_raphson_iv(V_mkt, S, K, tau, r, sigma0=0.5, tol=1e-10, max_iter=100):
    ...
    Scalar version. Returns sigma* or NaN if diverges.
    """
    sigma = sigma0
...
def batch_brent_iv(V_mkt, S, K, tau, r):
    """Brent's method applied element-wise."""
    n = len(V_mkt)
    results = np.full(n, np.nan)
    for i in range(n):
...
def batch_secant_iv(V_mkt, S, K, tau, r):
    """Secant method applied element-wise."""
    n = len(V_mkt)
    results = np.full(n, np.nan)
    for i in range(n):
...
def batch_bisection_iv(V_mkt, S, K, tau, r):
    """Bisection method applied element-wise."""
    n = len(V_mkt)
    results = np.full(n, np.nan)
    for i in range(n):

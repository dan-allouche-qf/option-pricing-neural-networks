"""Utility functions: data splitting, LHS sampling, plotting."""
...
def get_device():
    """Select compute device. MPS is slower than CPU for this model size."""
    if torch.cuda.is_available():
        return torch.device("cuda")
    # MPS is slower than CPU for small models - use CPU for training
...
def split_data(inputs, outputs, train_ratio=0.9, seed=42):
    ...
    Args:
        inputs: numpy array (n, d)
        outputs: numpy array (n, 1)
    ...
    X_train = inputs[idx[:split]]
    y_train = outputs[idx[:split]]
    X_test = inputs[idx[split:]]
    y_test = outputs[idx[split:]]

    return X_train, y_train, X_test, y_test


def split_data_3way(inputs, outputs, ratios=(0.8, 0.1, 0.1), seed=42):
    """Split data into train, validation and test sets (for Heston)."""
    rng = np.random.default_rng(seed)
    n = len(inputs)
    idx = rng.permutation(n)
    s1 = int(n * ratios[0])
    ...
    return (
        inputs[idx[:s1]], outputs[idx[:s1]],      # train
        inputs[idx[s1:s2]], outputs[idx[s1:s2]],  # val
        inputs[idx[s2:]], outputs[idx[s2:]],       # test
    )


def make_dataloader(X, y, batch_size=1024, shuffle=True, device=None):
    """Create a PyTorch DataLoader from numpy arrays.

    Data is kept on CPU for DataLoader compatibility.
    Transfer to device happens in the training loop only if needed.
    """
...
def plot_error_histogram(y_true, y_pred, title="Error distribution", filename=None):
    """Plot histogram of prediction errors with CDF overlay (like Figure 7)."""
...
def plot_scatter(y_true, y_pred, title="Predicted vs Actual", filename=None):
    """Scatter plot of predicted vs actual values (like Figure 8a, 9a)."""
    if isinstance(y_true, torch.Tensor):
...
def plot_training_history(train_losses, val_losses=None, title="Training loss", filename=None):
    """Plot training (and validation) loss history (like Figure 5)."""
...
def plot_training_history_sidebyside(history1, history2, titles, filename=None):
    """Plot two training histories side by side (Figure 5 of the article).

    Each history is a dict with 'train_losses' and optional 'val_losses'.
    """
    ...
    for ax, history, title in zip([ax1, ax2], [history1, history2], titles):
        ...
        ax.set_xlabel("Epoch", fontsize=12)
        ax.set_ylabel("log(MSE)", fontsize=12)
        ax.set_title(f"model loss", fontsize=13)
        ax.legend()
...
def plot_lr_schedule_comparison(histories_dict, filename=None):
    """Plot comparison of LR schedules (Figure 4 of the article).

    Args:
        histories_dict: {"Constant LR": [...], "Decay LR": [...], "Cyclical LR": [...]}
    """
...
def plot_lr_finder(lrs, losses, filename=None):
    """Plot LR range test results (Figure 3 of the article)."""
    fig, ax = plt.subplots(figsize=(8, 5))
...
def plot_dataset_size_study(cases, train_mse, test_mse, r2_test, train_std=None, test_std=None,
                            r2_std=None, filename=None):
    """Plot dataset size study with dual y-axis (Figure 6 of the article).

    Left y-axis: log(MSE) for training and testing.
    Right y-axis: R2 (%).
    """
...
def plot_pipeline_diagram(filename=None):
    """Plot the two-approach pipeline diagram (Figure 10 of the article)."""
